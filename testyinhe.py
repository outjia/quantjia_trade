...
def trade():
    map = {'MR_T2_B256_C2_E1000_Lclose_K5_XSGD': 'S2016-01-01.20181009.11.29.52.run',
...
if __name__ == '__main__':
    adjust_position(None, None)
    trade()
    # map = {'MR_T2_B256_C2_E1000_Lclose_K5_XSGD': ['S2016-01-01.20181009.11.29.52.run', '8080'], 'MR_T5_B256_C2_E2000_Lclose_K5_XSGD': ['S2015-01-03.20181114.11.21.34.run', '8080']}
    # file = os.path.realpath(__file__)
    # os.chdir(os.path.dirname(file))
    # model = sys.argv[1]
    # s = ServerProxy("http://127.0.0.1:" + map.get(model)[1])
    # run = map.get(model)[0]
    # data_str = s.predict_today_rpc(model, run, True)
    # stocks = pd.read_json(data_str, orient='records')
    #
    # if not os.path.exists(model): os.makedirs(model)
    # datapath = datetime.datetime.now().strftime(model + '/T%Y%m%d.txt')
    # stocks.to_csv(datapath)
